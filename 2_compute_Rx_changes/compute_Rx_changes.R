...
case_changes$either_change = case_changes$type_change | case_changes$dose_change
...
cntl_changes$either_change = cntl_changes$type_change | cntl_changes$dose_change
    ...
    case_by_subsets[nrow(case_by_subsets) + 1,] =
      c(PDD,
        ...
        sum(case_changes$dose_change[case_changes$PDD == PDD &
                                       case_changes$init_statin == statin]),
        sum(case_changes$either_change[case_changes$PDD == PDD &
                                       case_changes$init_statin == statin]))
  }
}
class(case_by_subsets$type_change) = "integer"
class(case_by_subsets$dose_change) = "integer"
class(case_by_subsets$either_change) = "integer"
    ...
    cntl_by_subsets[nrow(cntl_by_subsets) + 1,] =
      c(PDD,
        ...
        sum(cntl_changes$dose_change[cntl_changes$PDD == PDD &
                                       cntl_changes$init_statin == statin]),
        sum(cntl_changes$either_change[cntl_changes$PDD == PDD &
                                       cntl_changes$init_statin == statin]))
  }
}
class(cntl_by_subsets$type_change) = "integer"
class(cntl_by_subsets$dose_change) = "integer"
class(cntl_by_subsets$either_change) = "integer"
...
case_either_change = with(case_by_subsets, {
  out = matrix(nrow = nlevels(PDD), ncol = nlevels(init_statin),
               dimnames = list(levels(PDD), levels(init_statin)))
  out[cbind(PDD, init_statin)] = either_change
  out
})
case_either_change = cbind(case_either_change,
                         "Total" = apply(case_either_change, 1, sum))
case_either_change = rbind(case_either_change,
                         "Total" = apply(case_either_change, 2, sum))
write.csv(case_either_change, file = "data/case_either_change.csv")
...
cntl_either_change = with(cntl_by_subsets, {
  out = matrix(nrow = nlevels(PDD), ncol = nlevels(init_statin),
               dimnames = list(levels(PDD), levels(init_statin)))
  out[cbind(PDD, init_statin)] = either_change
  out
})
cntl_either_change = cbind(cntl_either_change,
